...
def get_weather_data():
    ...
    try:
        # Open-Meteo API endpoint untuk current weather dan forecast
        ...
        params = {
            ...
            'forecast_days': 8  # Untuk mendapatkan data 7 hari ke belakang
        }
        ...
        if response.status_code == 200:
            ...
            curah_hujan = current.get('precipitation', 0)  # mm
            weather_code = current.get('weather_code', 0)
            wind_speed = current.get('wind_speed_10m', 0)  # km/h
            wind_direction = current.get('wind_direction_10m', 0)
            
            # Konversi kondisi cuaca
            kondisi = get_weather_condition(weather_code)
            arah_angin = get_wind_direction(wind_direction)
            
            # Hitung akumulasi hujan dari hourly data
            # Ambil data hujan dari hourly untuk perhitungan kumulatif
            precipitation_hourly = hourly.get('precipitation', [])
            time_hourly = hourly.get('time', [])
            
            # Konversi ke DataFrame untuk memudahkan perhitungan
            if precipitation_hourly and time_hourly:
                df_hourly = pd.DataFrame({
                    'time': pd.to_datetime(time_hourly),
                    'precipitation': precipitation_hourly
                })
                
                # Filter untuk 7 hari terakhir (168 jam)
                now = pd.Timestamp.now(tz='Asia/Jakarta')
                df_7days = df_hourly[df_hourly['time'] >= (now - timedelta(days=7))]
                df_3days = df_hourly[df_hourly['time'] >= (now - timedelta(days=3))]
                
                cumulative_7day = df_7days['precipitation'].sum()
                cumulative_3day = df_3days['precipitation'].sum()
            else:
                cumulative_7day = 0
                cumulative_3day = 0
            ...
            weather_result = {
                ...
                'arah_angin_derajat': float(wind_direction),
                'cumulative_3day': float(cumulative_3day),
                'cumulative_7day': float(cumulative_7day),
...
def calculate_cumulative_rainfall(days=7):
    """Hitung kumulatif curah hujan dari historical data"""
    if len(st.session_state.rainfall_history) == 0:
        return 0
    
    df = pd.DataFrame(st.session_state.rainfall_history)
    cutoff_time = datetime.now() - timedelta(days=days)
    recent_data = df[df['timestamp'] >= cutoff_time]
    
    return recent_data['rainfall'].sum()
...
if weather_data['success']:
    ...
    risk_result = scorer.calculate_comprehensive_risk(
    ...
    with col1:
        ...
        <div class="score-card">
            ...
            <p style="color: #6c757d; font-size: 0.9em; margin: 5px 0;">
                Base Score: {risk_result['base_score']:.1f} | 
                Seasonal Factor: {risk_result['seasonal_factor']:.2f}x
            </p>
        </div>
        """, unsafe_allow_html=True)
...
st.caption("📡 Data real-time dari Open-Meteo API | 🤖 Telegram Auto-notification")
